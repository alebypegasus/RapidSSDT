...
class ACPIToolManager {
  ...
  void _initActionMap() {
    _actionMap = {
      ACPITable.ssdtHPET.name:
          ({PatchContext? context, Map<String, dynamic>? action}) => ssdt
              .ssdtHPET(devs: context?.devs, targetIrqs: context?.targetIrqs),
      ...
      ACPITable.ssdtUSBX.name:
          ({PatchContext? context, Map<String, dynamic>? action}) => ssdt
              .ssdtUSBX(prebuilt: _pb(context), usbxProps: context?.data ?? {}),
  ...
  Future<void> runPatches(
    ...
    for (final action in actions) {
      ...
      if (ssdtName != null && _hasGeneratedSsdt(ssdtName, outputFolder)) {
    ...
    while (stableCount < stableRounds) {
      ...
      bool changed =
          ...
          current.asMap().entries.any(
            (e) =>
                e.key < prev.length && e.value.modified != prev[e.key].modified,
          );
